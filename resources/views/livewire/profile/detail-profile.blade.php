<section class="section">
    ...
    <div class="mt-5">
        <div class="mb-4 text-center border-top pt-4">
            <h2 class="fw-bold mb-1">Daftar artikel {{ $user->name }}</h2>
        </div>
    ...
    @else
        <div class="row row-cols-1 row-cols-md-2 row-cols-lg-3 g-4">
            @foreach ($articles as $article)
                <div class="col d-flex">
                    <div class="card flex-fill blog-card rounded-3 overflow-hidden shadow-sm">
        ...
        <div class="text-center mt-3">
            @if ($totalFiltered > $articles->count())
                <button wire:click="loadMore" class="btn btn-outline-primary" wire:loading.attr="disabled">
                    <span wire:loading.remove wire:target="loadMore">Load More</span>
                    <span wire:loading wire:target="loadMore"><i class="fa fa-spinner fa-spin me-1"></i>
                        Loading...</span>
                </button>
            @else
